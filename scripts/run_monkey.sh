...
SNAPSHOT=$8 #emulator snapshot name
MONKEY_SEED=$9 #seed of monkey
...
do
    ...
    # emulator -port $avd_port -avd $AVD_NAME -snapshot snap_2022-04-20_10-31-54 -read-only $HEADLESS &
    emulator -port $avd_port -avd $AVD_NAME -snapshot $SNAPSHOT -read-only $HEADLESS &
    sleep 5
...
then
    ...
    # sleep 5 # wait for a few seconds before installation to avoid such error: "adb: connect error for write: closed"
    # adb -s $AVD_SERIAL install -g $APK_FILE &> $result_dir/install.log
    echo "** IGNORE INSTALL APP (${AVD_SERIAL})"
fi
...
adb -s $AVD_SERIAL shell date "+%Y-%m-%d-%H:%M:%S" >> $result_dir/monkey_testing_time_on_emulator.txt
if [[ $MONKEY_SEED -eq 0 ]]
then
    timeout $TEST_TIME adb -s $AVD_SERIAL shell monkey -p $app_package_name -v --throttle 2000 --pct-syskeys 0 --ignore-crashes --ignore-timeouts --ignore-security-exceptions --bugreport 1000000 2>&1 | tee $result_dir/monkey.log
else
    timeout $TEST_TIME adb -s $AVD_SERIAL shell monkey -s $MONKEY_SEED -p $app_package_name -v --throttle 2000 --pct-syskeys 0 --ignore-crashes --ignore-timeouts --ignore-security-exceptions --bugreport 1000000 2>&1 | tee $result_dir/monkey.log
fi
# add an additional package: -p com.android.camera
#timeout $TEST_TIME adb -s $AVD_SERIAL shell monkey -p $app_package_name -p com.android.camera -v --throttle 200 --ignore-crashes --ignore-timeouts --ignore-security-exceptions --bugreport 1000000 2>&1 | tee $result_dir/monkey.log
adb -s $AVD_SERIAL shell date "+%Y-%m-%d-%H:%M:%S" >> $result_dir/monkey_testing_time_on_emulator.txt
