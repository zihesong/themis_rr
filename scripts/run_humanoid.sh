...
do
    ...
    if [[ "$sta" =~ "offline" ]];
        then
            echo "########################offline, need to kill adb server########################"
            adb kill-server
            adb start-server
    fi
...
timeout $TEST_TIME droidbot -d $AVD_SERIAL -a $APK_FILE -o $result_dir -timeout 21600 -count 100000 -keep_app -keep_env -policy dfs_greedy -humanoid localhost:50405 -grant_perm -is_emulator 2>&1 | tee $result_dir/humandroid.log &
