...
adb -s $AVD_SERIAL push $APE_TOOL/ape.jar /data/local/tmp/
...
timeout $TEST_TIME adb -s $AVD_SERIAL shell CLASSPATH=/data/local/tmp/ape.jar /system/bin/app_process /data/local/tmp/ com.android.commands.monkey.Monkey -s 959274753518 -p $app_package_name --running-minutes 360 --ape sata 2>&1 | tee $result_dir/ape.log &
timeout $TEST_TIME bash screenshot.sh $result_dir
...
adb -s $AVD_SERIAL shell date "+%Y-%m-%d-%H:%M:%S" >> $result_dir/ape_testing_time_on_emulator.txt
# adb pull /sdcard/screenshots $result_dir/
